		...
		try:
			while True:
				...
				if response:
					...
					buffer = input('> ')
					buffer += '\n'
					self.socket.send(buffer.encode(encoding='UTF-8'))
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(
		...
		Example: netcat.py -t 192.168.1.108 -p 555 -l -u=mytest.txt #upload to file
		netcat.py -t 192.168.1.108 -p 5555 -l -c # command shell
		netcat.py -t 192.168.1.108 -p 555 -l -e=\"cat  /etc/passwd\" # execute command
		echo 'ABC' | ./netcat.py -t 192.168.1.198 -p 135 # echo text to server port 135
	...
	nc = NetCat(args, buffer.encode(encoding='UTF-8'))
	nc.run()
